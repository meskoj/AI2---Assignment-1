    ...
    (:action pick_up_tool
        :parameters (?g - gripper ?t - tool ?w - working_table)
        :precondition (and (at-tool ?t ?w) (at-robby ?w) (free ?g))
        :effect (and (picked_tool ?t ?g) (not(free ?g)))
    )

    (:action put_down_tool
        :parameters (?g - gripper ?t - tool ?w - working_table)
        :precondition (and (at-robby ?w) (picked_tool ?t ?g))
    ...
    (:action connect_switch_to_wire
       ...
       :effect (and (connected_switch_to_wire ?s ?wi)(free ?g1))
    )
    ...
    (:action connect_wire_to_motor
       ...
       :precondition (and (at ?wi ?w) (at-robby ?w) 
                        ...
                        (motor_aligned ?m) (regulator_mechanism_fixed_to_motor ?r) (fixed ?mb) (cable_tensioned ?c))
       :effect (and (connected_wire_to_motor ?wi ?m)(free ?g))
    )
